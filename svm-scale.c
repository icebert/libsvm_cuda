...
void output(int index, float value);
...
#define SKIP_ELEMENT\
	...
	feature_max = (float *)malloc((max_index+1)* sizeof(float));
	feature_min = (float *)malloc((max_index+1)* sizeof(float));
	...
	while(readline(fp)!=NULL)
	{
		...
		while(sscanf(p,"%d:%f",&index,&value)==2)
		{
