...
private:
	...
	{
		return tanhf(gamma*dot(x[i],x[j])+coef0);
	}
	...
	float *alpha, Solver::SolutionInfo* si)
{
	...
	for(i=0;i<l;i++)
		if(y[i] == +1)
		{
			alpha[i] = min(float(1.0),sum_pos);
			sum_pos -= alpha[i];
		}
		else
		{
			alpha[i] = min(float(1.0),sum_neg);
			sum_neg -= alpha[i];
		}
...
static void sigmoid_train(
	int l, const float *dec_values, const float *labels, 
	float& A, float& B)
{
	...
	for (iter=0;iter<max_iter;iter++)
	{
		...
		for (i=0;i<l;i++)
		{
			...
			if (fApB >= 0)
			{
				p=expf(-fApB)/(1.0+expf(-fApB));
				q=1.0/(1.0+expf(-fApB));
			}
			else
			{
				p=1.0/(1.0+expf(fApB));
				q=expf(fApB)/(1.0+expf(fApB));
			}
			d2=p*q;
...
static void svm_binary_svc_probability(
	const svm_problem *prob, const svm_parameter *param,
	float Cp, float Cn, float& probA, float& probB)
{
	int i;
	int nr_fold = 5;
	int *perm = Malloc(int,prob->l);
	float *dec_values = Malloc(float,prob->l);
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		else
		{
			...
			subparam.weight = Malloc(float,2);
...
static float svm_svr_probability(
	const svm_problem *prob, const svm_parameter *param)
{
	int i;
	int nr_fold = 5;
	float *ymv = Malloc(float,prob->l);
	float mae = 0;
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param)
{
	...
	else
	{
		...
		if (param->probability)
		{
			probA=Malloc(float,nr_class*(nr_class-1)/2);
			probB=Malloc(float,nr_class*(nr_class-1)/2);
		}
...
void svm_cross_validation(const svm_problem *prob, const svm_parameter *param, int nr_fold, float *target)
{
	int i;
	int *fold_start;
	int l = prob->l;
...
#pragma omp parallel for private(i) schedule(static)
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		{
			float *prob_estimates=Malloc(float,svm_get_nr_class(submodel));
			for(j=begin;j<end;j++)
...
int svm_grid_search(const struct svm_problem *prob, const struct svm_parameter *param, const struct grid_cg *grid, int nr_fold, float *target)
{
    int i, p;
	int l = prob->l;
    int nr_class;
    ...
    struct svm_problem *subprobs = Malloc(struct svm_problem, nr_fold);
    for(i=0; i<nr_fold; i++)
    {
        int begin = fold_start[i];
        int end = fold_start[i+1];
        ...
        subprobs[i].x = Malloc(struct svm_node*, subprobs[i].l);
        subprobs[i].y = Malloc(float, subprobs[i].l);
        
        k=0;
        for(j=0; j<begin; j++)
        {
            subprobs[i].x[k] = perm[j];
    ...
    struct svm_model *submodels = Malloc(struct svm_model, grid->l * nr_fold);
    ...
    //for (p=0; p<grid->l; p++)
    //    for (i=0; i<nr_fold; i++)
    //        submodels[p*nr_fold+i] = svm_train(&(subprobs[i]), &(params[p]));
    int res = cuda_svm_train(prob, subprobs, params, grid->l, nr_fold, submodels);
...
#pragma omp parallel for private(i) schedule(dynamic)
            ...
            for(i=0; i<nr_fold; i++)
            {
                ...
                float *local_target = target + p*l;
                if(param->probability && (param->svm_type == C_SVC || param->svm_type == NU_SVC))
                {
                    float *prob_estimates = Malloc(float, svm_get_nr_class(&(submodels[p*nr_fold+i])));
                    for(j=begin; j<end; j++)
                        local_target[perm[j]] = svm_predict_probability(&(submodels[p*nr_fold+i]), prob->x[perm[j]], prob_estimates);
                    free(prob_estimates);
                }
                else
                    for(j=begin; j<end; j++)
                        local_target[perm[j]] = svm_predict(&(submodels[p*nr_fold+i]), prob->x[perm[j]]);
                struct svm_model *model = Malloc(struct svm_model, 1);
                *model = submodels[p*nr_fold+i];
...
float svm_get_svr_probability(const svm_model *model)
{
...
float svm_predict(const svm_model *model, const svm_node *x)
{
	...
	if(model->param.svm_type == ONE_CLASS ||
	   model->param.svm_type == EPSILON_SVR ||
	   model->param.svm_type == NU_SVR)
		dec_values = Malloc(float, 1);
	else 
		dec_values = Malloc(float, nr_class*(nr_class-1)/2);
	float pred_result = svm_predict_values(model, x, dec_values);
	free(dec_values);
	return pred_result;
}

float svm_predict_probability(
	const svm_model *model, const svm_node *x, float *prob_estimates)
{
